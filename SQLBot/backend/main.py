...
def mount_xpack_static_assets(app: FastAPI):
    if any(getattr(route, "path", None) == "/xpack_static" for route in app.routes):
        return
    xpack_static_dir = os.path.join(os.path.dirname(sqlbot_xpack.__file__), "static")
    if os.path.isdir(xpack_static_dir):
        app.mount("/xpack_static", StaticFiles(directory=xpack_static_dir), name="xpack-static")
...
if __name__ == "__main__":
    ...
    uvicorn.run("main:app", host="0.0.0.0", port=8000, reload=os.getenv("SQLBOT_RELOAD", "false").lower() == "true")
