...
@router.get("/runtime", response_model=list[AiModelGridItem], include_in_schema=False)
async def runtime_models(session: SessionDep, current_user: CurrentUser):
    statement = select(
        AiModelDetail.id,
        AiModelDetail.name,
    ...
    ).order_by(AiModelDetail.default_model.desc(), AiModelDetail.name, AiModelDetail.create_time)
