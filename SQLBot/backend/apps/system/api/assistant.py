...
async def validator(
    ...
    external_user_id: Optional[str] = Query(None),
    workspace_oid: Optional[int] = Query(None),
):
    if not id:
        raise Exception('miss assistant id')
    ...
    normalized_workspace_oid = workspace_oid if workspace_oid and workspace_oid > 0 else assistant_oid

    stable_user_id = virtual
    if normalized_external_user_id:
        raw_key = f"{id}:{normalized_workspace_oid}:{normalized_external_user_id}"
        digest = hashlib.sha256(raw_key.encode('utf-8')).digest()
        stable_user_id = int.from_bytes(digest[:8], 'big') & ((1 << 63) - 1)
        if stable_user_id == 0:
            stable_user_id = 1
    ...
    assistantDict = {
        "id": stable_user_id,
        "account": f"sqlbot-inner-assistant:{normalized_external_user_id}" if normalized_external_user_id else 'sqlbot-inner-assistant',
        "oid": normalized_workspace_oid,
        "assistant_id": id
    }
...
async def ds(session: SessionDep, current_assistant: CurrentAssistant):
    ...
    if current_assistant.type in (1, 4):
