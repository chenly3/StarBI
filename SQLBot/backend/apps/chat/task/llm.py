...
from apps.ai_model.model_factory import LLMConfig, LLMFactory, get_model_config
...
def build_learning_context_block(
        *,
        resource_name: str = '',
        terminology_mappings: Any = None,
        sample_values: Any = None,
        sql_examples: Any = None,
) -> str:
    def stringify_value(value: Any) -> str:
        ...
        if isinstance(value, dict):
            ...
            for key, raw_value in value.items():
                rendered_value = stringify_value(raw_value)
                if rendered_value:
    ...
    def format_term_entry(entry: Any) -> str:
        if isinstance(entry, dict):
            term = stringify_value(
                entry.get('term') or entry.get('word') or entry.get('alias') or entry.get('name'))
            target = stringify_value(
                entry.get('target') or entry.get('field') or entry.get('value') or entry.get('mapping'))
            if term and target:
                return f'{term} => {target}'
        return stringify_value(entry)

    def format_sample_entry(entry: Any) -> str:
        if isinstance(entry, dict):
            field_name = stringify_value(
                entry.get('field') or entry.get('field_name') or entry.get('name'))
            rendered_value = stringify_value(
                entry.get('values') if entry.get('values') is not None
                else entry.get('sample_values') if entry.get('sample_values') is not None
                else entry.get('value') if entry.get('value') is not None
                else entry.get('samples')
            )
            if field_name and rendered_value:
                return f'{field_name}: {rendered_value}'
        return stringify_value(entry)

    def format_sql_example_entry(entry: Any) -> str:
        if isinstance(entry, dict):
            question = stringify_value(entry.get('question') or entry.get('title'))
            sql_text = stringify_value(entry.get('sql') or entry.get('query'))
            if question and sql_text:
                return f'问题: {question} | SQL: {sql_text}'
            if sql_text:
    ...
    rendered_resource_name = stringify_value(resource_name)
    if rendered_resource_name:
        sections.append(('资源名称', [rendered_resource_name]))

    rendered_terms = unique_lines([format_term_entry(entry) for entry in (terminology_mappings or [])])
    if rendered_terms:
        sections.append(('术语映射', rendered_terms))

    rendered_samples = unique_lines([format_sample_entry(entry) for entry in (sample_values or [])])
    if rendered_samples:
        sections.append(('值样本', rendered_samples))

    rendered_sql_examples = unique_lines([format_sql_example_entry(entry) for entry in (sql_examples or [])])
    if rendered_sql_examples:
        sections.append(('SQL 示例', rendered_sql_examples))
    ...
    lines = [
        '<learning-context>',
        '以下是当前数据资源的运行时学习资产，请优先用于语义理解、字段映射、值匹配和SQL生成校准；若与表结构、权限或显式规则冲突，以后者为准。',
    ]
...
def build_learning_context_messages(context_block: str) -> list[tuple[str, str]]:
    if not context_block:
        return []
    return [
        (
            context_block,
            '我已确认当前数据资源的学习资产，会优先参考其中的资源名称、术语映射、值样本和SQL示例。',
        )
    ]


def load_runtime_learning_context_block(session: Session, datasource_id: Optional[int]) -> str:
    if not datasource_id:
        return ''

    def unwrap_learning_result(row: Any) -> Any:
        if isinstance(row, QueryResourceLearningResult):
            return row
    ...
    for resource_id in [f'datasource:{datasource_id}', str(datasource_id)]:
        ...
        learning_results.extend([unwrap_learning_result(row) for row in session.exec(statement).all()])
    ...
    semantic_profile = learning_result.semantic_profile if isinstance(learning_result.semantic_profile, dict) else {}
    terminology_mappings = (
            semantic_profile.get('terminology_mappings')
            or semantic_profile.get('terminologies')
            or semantic_profile.get('term_mappings')
            or semantic_profile.get('terms')
            or []
    )
    sql_examples = (
            semantic_profile.get('sql_examples')
            or semantic_profile.get('example_sqls')
            or semantic_profile.get('examples')
            or []
    )
    return build_learning_context_block(
        resource_name=semantic_profile.get('dataset_name') or semantic_profile.get('resource_name') or semantic_profile.get('name') or '',
        terminology_mappings=terminology_mappings,
        sample_values=learning_result.sample_values or [],
...
class LLMService:
    ...
    def __init__(self, session: Session, current_user: CurrentUser, chat_question: ChatQuestion,
        ...
        # Embedded StarBI/DataEase integration can force datasource switching while
        # keeping the same chat history. When the caller provides a datasource_id
        # different from the persisted chat.datasource, clear datasource so the
        # service will re-select from the current embedded scope (certificate).
        if (
            current_assistant
            and current_assistant.type in dynamic_ds_types
            and chat.datasource
            and chat_question.datasource_id
            and chat.datasource != chat_question.datasource_id
            and not chat_question.regenerate_record_id
        ):
...
{last_execute_sql_error}
    ...
    async def create(cls, *args, **kwargs):
        chat_question: ChatQuestion = args[2]
        config: LLMConfig = await get_model_config(
            model_id=chat_question.ai_modal_id,
            model_name=chat_question.ai_modal_name
        )
    ...
    def init_messages(self, session: Session):
        ...
        learning_context_block = load_runtime_learning_context_block(
            session,
            getattr(self.ds, 'id', None),
        )
        for learning_context_content, learning_context_ack in build_learning_context_messages(
                learning_context_block):
            self.sql_message.append(HumanPromptMessage(content=learning_context_content))
            self.sql_message.append(AIPromptMessage(content=learning_context_ack))
    ...
    def try_apply_learning_sql_override(self, _session: Session) -> dict[str, Any] | None:
        datasource_id = getattr(self.ds, 'id', None)
        question_text = (getattr(self.chat_question, 'question', None) or getattr(self.record, 'question', None) or '').strip()
        if not datasource_id or not question_text:
            return None
        ...
        for resource_id in resource_ids:
            ...
            self.current_logs[OperationEnum.GENERATE_SQL] = start_log(
                ...
                full_message=[
                    {'type': msg.type,
                     ...
                     'content': msg.content} for msg in self.sql_message
                ],
            )
            ...
            self.current_logs[OperationEnum.GENERATE_SQL] = end_log(
                ...
                reasoning_content='命中问数资源学习修正，跳过大模型 SQL 生成。',
                token_usage={},
            )
            ...
            record_patch_apply_log(
                _session,
                resource_id=resource_id,
                chat_record_id=self.record.id,
                trace_id=f'qrl_apply_{self.record.id}',
                question_text=question_text,
                pre_sql=None,
    ...
    @staticmethod
    def normalize_dynamic_temp_table_sql(sql_text: str) -> str:
        if not sql_text:
            return sql_text
        # Some LLM responses wrap temp sub-query placeholders as:
        # `db`.`(select * from sqlbot_dynamic_temp_table_xxx)`
        # which is invalid SQL. Normalize this to:
        # (select * from sqlbot_dynamic_temp_table_xxx)
        db_wrapped_pattern = re.compile(
            r'`[^`]+`\.`\(\s*(select\s+\*\s+from\s+sqlbot_dynamic_temp_table_[a-zA-Z0-9_]+)\s*\)`',
            flags=re.IGNORECASE,
        )
        quoted_subquery_pattern = re.compile(
            r'`\(\s*(select\s+\*\s+from\s+sqlbot_dynamic_temp_table_[a-zA-Z0-9_]+)\s*\)`',
            flags=re.IGNORECASE,
        )
        normalized = db_wrapped_pattern.sub(r'(\1)', sql_text)
        normalized = quoted_subquery_pattern.sub(r'(\1)', normalized)
        return normalized

    @classmethod
    def apply_dynamic_temp_table_replacements(cls, sql_text: str, dynamic_sql_result: dict[str, str]) -> str:
        normalized_sql = cls.normalize_dynamic_temp_table_sql(sql_text)
        if not normalized_sql or not dynamic_sql_result:
            return normalized_sql

        clean_mappings = {
            table_name.strip(): subsql
            for table_name, subsql in dynamic_sql_result.items()
            if table_name and table_name.strip() and subsql
        }
        if not clean_mappings:
            return normalized_sql

        lower_mappings = {table_name.lower(): subsql for table_name, subsql in clean_mappings.items()}

        def resolve_subsql(table_name: str) -> str | None:
            if table_name in clean_mappings:
                return clean_mappings[table_name]
            return lower_mappings.get(table_name.lower())

        replaced_sql = normalized_sql
        for origin_table, subsql in clean_mappings.items():
            table_key = re.escape(origin_table)
            table_pattern = re.compile(
                rf'(?is)select\s+\*\s+from\s+(?:`?[a-zA-Z0-9_]+`?\.)?`?sqlbot_dynamic_temp_table_{table_key}`?'
            )
            replaced_sql = table_pattern.sub(subsql, replaced_sql)

        generic_pattern = re.compile(
            r'(?is)select\s+\*\s+from\s+(?:`?[a-zA-Z0-9_]+`?\.)?`?sqlbot_dynamic_temp_table_([^`)\n]+)`?'
        )

        def generic_replacer(match: re.Match) -> str:
            table_name = match.group(1).strip()
            subsql = resolve_subsql(table_name)
            return subsql if subsql else match.group(0)

        replaced_sql = generic_pattern.sub(generic_replacer, replaced_sql)
        return replaced_sql
    ...
    def build_scope_label(self):
        if self.ds and getattr(self.ds, 'name', None):
            return self.ds.name
        if self.chat_question.datasource_id:
            return f'数据源 {self.chat_question.datasource_id}'
        return '当前问数范围'

    def build_query_interpretation_payload(self):
        return infer_query_interpretation(self.chat_question.question or '')

    def build_clarification_event(self):
        interpretation = self.build_query_interpretation_payload()
        candidate_datasets = [self.ds.name] if self.ds and getattr(self.ds, 'name', None) else []
        decision = should_request_clarification(
            question=self.chat_question.question or '',
            candidate_metrics=infer_metric_candidates(
                self.chat_question.question or '',
                interpretation.get('metric') or [],
            ),
            candidate_dimensions=infer_dimension_candidates(
                self.chat_question.question or '',
                interpretation.get('dimension') or [],
            ),
            candidate_datasets=candidate_datasets,
            detected_time_range=detect_time_range(self.chat_question.question or ''),
        )
    ...
    def build_query_interpretation_event(self):
        return {
            'type': 'query_interpretation',
            'content': self.build_query_interpretation_payload(),
        }

    def build_execution_summary_event(self, failure_stage: Optional[str] = None):
        interpretation = self.build_query_interpretation_payload()
        return {
            'type': 'execution_summary',
            'content': build_execution_summary(
                interpretation=interpretation,
                scope_label=self.build_scope_label(),
                datasource_label=self.ds.name if self.ds and getattr(self.ds, 'name', None) else '',
    ...
    def run_task(self, in_chat: bool = True, stream: bool = True,
        ...
        try:
            ...
            clarification_event = self.build_clarification_event()
            if clarification_event and in_chat:
                yield 'data:' + orjson.dumps(clarification_event).decode() + '\n\n'
                return
            ...
            # generate sql
            learning_override = self.try_apply_learning_sql_override(_session)
            full_sql_text = ''
            if learning_override:
                full_sql_text = learning_override['sql_answer']
                if in_chat:
                    yield 'data:' + orjson.dumps(
                        {'content': full_sql_text, 'reasoning_content': '',
            ...
            if sqlbot_temp_sql_text and assistant_dynamic_sql:
                if dynamic_sql_result:
                    dynamic_sql_result = dict(dynamic_sql_result)
                    dynamic_sql_result.pop('sqlbot_temp_sql_text', None)
                real_execute_sql = self.apply_dynamic_temp_table_replacements(assistant_dynamic_sql, dynamic_sql_result)
            ...
            if in_chat:
                ...
                yield 'data:' + orjson.dumps(self.build_execution_summary_event()).decode() + '\n\n'
            else:
                if stream:
            ...
            failure_stage = infer_failure_stage(
                'db-connection-err'
                if isinstance(e, SQLBotDBConnectionError)
                else 'exec-sql-err'
                if isinstance(e, SQLBotDBError)
                else None,
                str(e),
            )
            if _session:
            ...
            if in_chat:
                yield 'data:' + orjson.dumps(self.build_query_interpretation_event()).decode() + '\n\n'
                yield 'data:' + orjson.dumps(
                    self.build_execution_summary_event(failure_stage=failure_stage)
                ).decode() + '\n\n'
    ...
    def validate_history_ds(self, session: Session):
        ...
        if self.current_assistant and self.current_assistant.type == 4:
            try:
                if not _ds or not getattr(_ds, 'id', None):
                    raise SingleMessageError('chat.ds_is_invalid')
                if not self.out_ds_instance:
                    self.out_ds_instance = AssistantOutDsFactory.get_instance(self.current_assistant)
                current_ds = self.out_ds_instance.get_ds(_ds.id)
                if not current_ds:
                    raise SingleMessageError('chat.ds_is_invalid')
                self.ds = current_ds
                return
