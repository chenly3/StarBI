    ...
    try:
        ...
        upsert_sqlbot_new_snapshot(
            ...
            SqlbotNewSnapshotUpsert(
                ...
                active_source_ids=payload.source_ids,
                active_combination_id=payload.combination_id,
                active_combination_name=payload.combination_name,
...
async def sqlbot_new_snapshot(
    ...
    try:
        snapshot = upsert_sqlbot_new_snapshot(session, current_user, chat_id, payload)
        session.commit()
...
async def start_chat(session: SessionDep, current_user: CurrentUser, current_assistant: CurrentAssistant,
    ...
    try:
        if current_user is None:
            current_user = get_assistant_user(id=1)
