...
def _get_chat_with_user_ownership(session: SessionDep, current_user: CurrentUser, chat_id: int) -> Chat:
    chat = session.get(Chat, chat_id)
    if not chat:
        raise Exception(f"Chat with id {chat_id} not found")
    if chat.create_by != current_user.id:
        raise Exception(f"Chat with id {chat_id} not owned by the current user")

    oid = current_user.oid if current_user.oid is not None else 1
    if chat.oid != oid:
        raise Exception(f"Chat with id {chat_id} does not belong to current workspace")
...
def upsert_sqlbot_new_snapshot(
    ...
    _get_chat_with_user_ownership(session, current_user, chat_id)
    ...
    if snapshot is None:
        ...
        for key, value in payload.model_dump().items():
            if key == "latest_question" and not value and snapshot.latest_question:
                continue
            if key == "latest_record_id" and value is None and snapshot.latest_record_id is not None:
                continue
...
def list_sqlbot_new_histories(session: SessionDep, current_user: CurrentUser) -> list[SqlbotNewHistoryEntry]:
    stmt = (
        ...
        .where(
            and_(
                ...
                ChatSessionSnapshot.latest_question.isnot(None),
                ChatSessionSnapshot.latest_question != "",
            )
        )
    ...
    return [
        SqlbotNewHistoryEntry(
            ...
            source_ids=snapshot.active_source_ids or [],
            combination_id=snapshot.active_combination_id or "",
            combination_name=snapshot.active_combination_name or "",
            ...
            latest_question=snapshot.latest_question or "",
...
def get_sqlbot_new_context(
    ...
    events = list(session.exec(stmt).scalars().all())
...
def resolve_chat_datasource(
        session: SessionDep,
        chat: Chat,
        current_assistant: CurrentAssistant,
        trans: Trans = None,
) -> Optional[Union[CoreDatasource, AssistantOutDsSchema]]:
    ...
    try:
        if current_assistant and current_assistant.type in dynamic_ds_types:
            ...
            return out_ds_instance.get_ds(chat.datasource, trans)
        return session.get(CoreDatasource, chat.datasource)
    except Exception as e:
        SQLBotLogUtil.warning(f"Resolve chat datasource failed: {e}")
        return None
...
def get_chat_with_records(session: SessionDep, chart_id: int, current_user: CurrentUser,
    ...
    ds = resolve_chat_datasource(session, chat, current_assistant, trans)
    ...
    result = merge_child_records_into_parents(result)
...
def create_chat(session: SessionDep, current_user: CurrentUser, create_chat_obj: CreateChat,
    ...
    if create_chat_obj.datasource:
        ...
        if current_assistant and current_assistant.type in (1, 3, 4):
