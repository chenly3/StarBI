...
def expand_loopback_origins(origins: list[str]) -> list[str]:
    expanded: list[str] = []
    seen: set[str] = set()
    local_dev_ports = (5173, 8000, 8100)
    ...
    for origin in origins:
        ...
        for alias in ("localhost", "127.0.0.1"):
            ...
            add_origin(urllib.parse.urlunsplit((parsed.scheme, netloc, parsed.path, parsed.query, parsed.fragment)))

        # Local StarBI/DataEase + SQLBot integration often runs across 5173/8000/8100
        # during development. Mirror loopback aliases across these ports so POST requests
        # with assistant headers can pass CORS preflight in the split local setup.
        if parsed.port in local_dev_ports:
            for alias in ("localhost", "127.0.0.1"):
...
class Settings(BaseSettings):
    ...
    def all_cors_origins(self) -> list[str]:
        configured_origins = [str(origin).rstrip("/") for origin in self.BACKEND_CORS_ORIGINS]
        configured_origins.append(self.FRONTEND_HOST.rstrip("/"))
    ...
    DEFAULT_EMBEDDING_MODEL: str = 'iic/nlp_gte_sentence-embedding_chinese-base'
    EMBEDDING_PROVIDER: Literal["local", "vllm"] = "local"
    VLLM_EMBEDDING_API_BASE: str = "http://127.0.0.1:8002/v1"
    VLLM_EMBEDDING_API_KEY: str = "EMPTY"
    VLLM_EMBEDDING_TIMEOUT: int = 60
