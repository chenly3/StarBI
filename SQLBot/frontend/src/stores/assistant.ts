...
import { resolveParentOrigin } from '@/utils/utils'
...
interface AssistantEmbeddedScope {
  entryScene: string
  datasetIds: string[]
  datasourceId: string
...
const parseEmbeddedScope = (certificate?: string): AssistantEmbeddedScope => {
  const scope = createEmptyEmbeddedScope()
  if (!certificate) {
    return scope
  }

  try {
    const certificateItems = JSON.parse(certificate)
    if (!Array.isArray(certificateItems)) {
      return scope
    }

    certificateItems.forEach(item => {
      if (item?.target !== 'param' || !item?.key) {
        return
      }
      const value = String(item?.value ?? '').trim()
      scope.params[item.key] = value
    })

    scope.entryScene = scope.params.entryScene || ''
    scope.datasourceId = scope.params.dsId || ''
    scope.datasetIds = (scope.params.datasetIds || '')
      .split(',')
      .map((item: string) => item.trim())
      .filter(Boolean)
  } catch (error) {
    console.error('parse embedded scope failed', error)
  }
...
export const AssistantStore = defineStore('assistant', {
  state: (): AssistantState => {
    return {
      ...
      embeddedScope: createEmptyEmbeddedScope(),
  ...
  getters: {
    ...
    getExternalComposer(): boolean {
      return !!this.externalComposer
    },
    getEmbeddedScope(): AssistantEmbeddedScope {
      return this.embeddedScope
    },
    getHasEmbeddedScope(): boolean {
      return (
        this.embeddedScope.datasetIds.length > 0 ||
        !!this.embeddedScope.datasourceId ||
        !!this.embeddedScope.entryScene
      )
    },
  },
  actions: {
    refreshCertificate(requestUrl?: string) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (this.peddingStatus !== 1) {
          ...
          window.parent.postMessage(readyData, resolveParentOrigin(this.hostOrigin) || '*')
        }
      })
    },
    ...
    setCertificate(certificate: string) {
      ...
      this.embeddedScope = parseEmbeddedScope(certificate)
    },
    setType(type: number) {
      this.type = type
    },
    ...
    setSelectedModel(modelId?: string, modelName?: string) {
      this.selectedModelId = modelId || ''
      this.selectedModelName = modelName || ''
    },
    setExternalComposer(externalComposer?: boolean) {
      this.externalComposer = !!externalComposer
    },
    async setChat() {
      if (!this.assistant) {
        return null
