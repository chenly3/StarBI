<template>
  ...
  <el-container
    ...
    :class="{ 'starbi-embedded-stream': embeddedExternalComposer }"
  >
    <el-aside
    ...
    <el-container :loading="loading">
          ...
          <div class="welcome-content">
            <template v-if="isCompletePage">
              <div class="greeting">
                ...
                ></el-icon>
                {{ appearanceStore.pc_welcome ?? '你好，我是 StarBI' }}
              </div>
              <div class="sub">
                {{
      ...
      <el-footer
        v-if="
          !embeddedExternalComposer &&
          (computedMessages.length > 0 || (!isCompletePage && !selectAssistantDs))
        "
        class="chat-footer"
      >
...
const embeddedExternalComposer = computed(
  () =>
    assistantStore.getAssistant &&
    !!props.pageEmbedded &&
    assistantStore.getPageEmbedded &&
    assistantStore.getExternalComposer
)
// const autoDs = computed(() => assistantStore.getAssistant && assistantStore.getAutoDs)
const skipAssistantDatasourceSelection = computed(() => {
  return (
    assistantStore.getAssistant &&
    !!props.pageEmbedded &&
    assistantStore.getPageEmbedded &&
    assistantStore.getHasEmbeddedScope
  )
})
const selectAssistantDs = computed(() => {
  return (
    assistantStore.getAssistant &&
    !assistantStore.getAutoDs &&
    !skipAssistantDatasourceSelection.value
  )
})
const autoCreateAssistantChat = computed(() => {
  return (
    assistantStore.getAssistant &&
    !!props.pageEmbedded &&
    assistantStore.getPageEmbedded &&
...
const createNewChat = async () => {
  ...
  goEmpty()
  if (autoCreateAssistantChat.value) {
...
async function quickAskFromExternal(question: string) {
...
.chat-container {
  ...
  &.starbi-embedded-stream {
    ...
    :deep(.ai-avatar .el-icon),
    :deep(.ai-avatar .ed-icon) {
      font-size: 18px;
      color: #ffffff;
    }

    :deep(.base-answer-block .thinking-btn) {
      height: 28px;
      padding: 4px 10px;
      border-radius: 999px;

      --ed-button-bg-color: rgba(243, 244, 246, 1);
      --ed-button-hover-bg-color: rgba(229, 231, 235, 1);
      --ed-button-active-bg-color: rgba(229, 231, 235, 1);
      --ed-button-border-color: rgba(229, 231, 235, 1);
      --ed-button-hover-border-color: rgba(229, 231, 235, 1);
      --ed-button-active-border-color: rgba(229, 231, 235, 1);
    }

    :deep(.base-answer-block .thinking-btn-inner) {
      font-size: 13px;
      color: rgba(78, 89, 105, 1);
    }

    :deep(.base-answer-block .reasoning-content) {
      margin-top: 10px;
      padding-left: 12px;
      border-left-color: rgba(229, 231, 235, 1);
    }

    :deep(.base-answer-block .reasoning) {
      color: rgba(107, 114, 128, 1) !important;
    }

    :deep(.base-answer-block .reasoning .markdown-body) {
      color: rgba(107, 114, 128, 1) !important;
    }
    ...
    .welcome-content .assistant-desc {
      align-items: flex-start;
    }

    .welcome-content .assistant-desc .i-am {
      margin: 8px 0 6px;
      padding: 0;
      font-size: 18px;
      line-height: 28px;
    ...
    .welcome-content .assistant-desc .i-can {
      max-width: none;
      padding: 0;
      margin-bottom: 0;
      text-align: left;
