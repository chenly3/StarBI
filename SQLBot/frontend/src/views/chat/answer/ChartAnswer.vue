...
const sendMessage = async () => {
  ...
  try {
    ...
    const selectedModelId = Number.parseInt(assistantStore.getSelectedModelId, 10)
    const embeddedDatasourceId = Number.parseInt(assistantStore.getEmbeddedScope?.datasourceId || '', 10)
    const param = {
      question: currentRecord.question,
      chat_id: _currentChatId.value,
      ai_modal_id: Number.isNaN(selectedModelId) ? undefined : selectedModelId,
      ai_modal_name: assistantStore.getSelectedModelName || undefined,
      datasource_id: Number.isNaN(embeddedDatasourceId) ? undefined : embeddedDatasourceId,
    }
