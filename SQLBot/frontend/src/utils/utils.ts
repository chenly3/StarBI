...
export const setTitle = (title?: string) => {
  document.title = title || STARBI_BRAND_NAME
}

export const STARBI_BRAND_NAME = 'StarBI'
...
export const applyEmbeddedStyleTokens = (styleTokens: Record<string, any> = {}) => {
  const root = document.documentElement
  const body = document.body
  const normalizedTokens = {
    brandName: styleTokens.brandName || STARBI_BRAND_NAME,
    primaryColor: styleTokens.primaryColor || '#3370ff',
    pageBg: styleTokens.pageBg || '#f5f6f7',
    surfaceBg: styleTokens.surfaceBg || '#ffffff',
    borderColor: styleTokens.borderColor || 'rgba(31, 35, 41, 0.12)',
    textColor: styleTokens.textColor || '#1f2329',
    mutedColor: styleTokens.mutedColor || '#646a73',
    radius: styleTokens.radius || '6px',
    fontFamily:
      styleTokens.fontFamily || '"PingFang SC", "Microsoft YaHei", "Helvetica Neue", sans-serif',
  }

  setCurrentColor(normalizedTokens.primaryColor, root)
  root.style.setProperty('--starbi-primary-color', normalizedTokens.primaryColor)
  root.style.setProperty('--starbi-page-bg', normalizedTokens.pageBg)
  root.style.setProperty('--starbi-surface-bg', normalizedTokens.surfaceBg)
  root.style.setProperty('--starbi-border-color', normalizedTokens.borderColor)
  root.style.setProperty('--starbi-text-color', normalizedTokens.textColor)
  root.style.setProperty('--starbi-muted-color', normalizedTokens.mutedColor)
  root.style.setProperty('--starbi-radius', normalizedTokens.radius)
  root.style.setProperty('--de-custom_font', normalizedTokens.fontFamily)

  if (body) {
    body.style.margin = '0'
    body.style.background = normalizedTokens.pageBg
    body.style.color = normalizedTokens.textColor
    body.style.fontFamily = normalizedTokens.fontFamily
  }

  return normalizedTokens
}

export const isStarBIPageMode = (pageMode: unknown) => {
  return String(pageMode || '').toLowerCase() === 'starbi'
}

export const resolveParentOrigin = (origin?: string) => {
  if (origin) {
    return origin
  }
  ...
  try {
    ...
    console.error('resolve parent origin failed', error)
    return ''
  }
}
