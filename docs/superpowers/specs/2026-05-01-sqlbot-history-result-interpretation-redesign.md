...
本设计用于约束后续实现：对标 Quick BI 小Q问数的产品思路，把问数结果从“万能大卡片”改为“事实结果区 + 洞察解读区 + 追问区”的稳定信息架构。
...
- 推荐追问属于下一轮提问入口，不插在事实结果和洞察解读之间。
- 用户能看懂每个区域的来源：事实来自 SQLBot 取数，解读来自 LLM 基于本轮结果生成。
- 旧数据可以自然降级：没有保存解读时显示空态和“生成解读”按钮，不自动补生成。
...
推荐追问不放在“事实结果区”和“洞察解读区”之间。默认顺序为事实结果、洞察解读、趋势预测、推荐追问。这样用户先确认事实，再阅读洞察，最后决定是否继续追问。

问数成功后返回的文本需要分层展示：

- 问数回答文本属于事实结果区，用于解释本轮查到了什么、图表/表格表达了什么。
- 执行/推理文本属于事实结果区的执行过程，用于说明问题理解、字段识别、SQL 生成、取数步骤。
- 数据解读文本属于洞察解读区，只展示由 `analysis` 生成并保存的业务洞察、原因分析和建议。

Quick BI 式的“做数据解读”更接近一个自动提问入口：用户点击后，系统以当前事实结果为上下文发起解读任务，再把结果写入洞察解读区。它不是把问数回答文本改名成数据解读，也不是在历史恢复时自动重跑解读。
...
- 右侧或下方为洞察解读面板。大屏宽度足够时使用右侧面板；宽度不足时降为图表下方独立卡片。
- 推荐追问固定在本轮回答底部，不和解读面板抢视觉主次。
...
5. 是否自动生成解读由产品开关决定，但必须作为明确策略执行，不能由组件挂载、watch 数据变化或历史恢复隐式触发。

建议默认策略：

- 新问题默认不自动生成数据解读，展示“生成解读”按钮。
- 如果后续产品决定默认开启解读，必须只在新问题提交链路触发一次，并写入 `analysis_status = generating`。

### 数据解读生成

1. 用户点击“生成解读”或“重新解读”。
2. 前端调用 SQLBot analysis 流接口。
3. 流式内容进入洞察解读区，不覆盖事实结果区。
4. 生成完成后后端保存 `analysis`、`analysis_reasoning_content`、`analysis_record_id`、耗时、Token。
5. 历史恢复时直接展示这些已保存字段。
...
- `GET /chat/{chat_id}/sqlbot-new/context` 或会话详情接口：返回完整会话快照，包含每轮事实结果和已保存解读。
...
- `SqlbotConversationTurn.vue`：组合一轮问答，不包含生成逻辑。

事件约束：

- `SqlbotInsightPanel` 只通过用户点击发出 `generate-analysis`。
...
- 点击历史对话且历史中无解读时，页面展示“本轮尚未生成数据解读”，网络请求中不得出现 `/analysis`。
- 点击“生成解读”后，只调用一次 `/analysis`，流式内容进入洞察解读区，图表/表格不被覆盖。
- 刷新页面恢复当前会话时，不重复触发 `/analysis`。
- 推荐追问出现在本轮底部，不插入事实结果与洞察解读之间。
- 推荐追问卡片可以包含“做数据解读/看趋势预测/继续追问”等自动提问入口，但点击后必须作为明确用户动作触发。
...
- 新建问数，等待事实结果生成。
- 点击“生成解读”，确认解读区域流式生成并持久化。
- 刷新页面，确认保存的解读直接恢复。
- 点击历史对话，确认无 `/analysis` 请求。
- 选择一个没有解读的旧历史，确认只展示空态。
- 点击推荐追问，确认生成下一轮问题，而不是影响上一轮解读。
