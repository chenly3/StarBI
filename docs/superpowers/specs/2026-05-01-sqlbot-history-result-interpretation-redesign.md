...
本设计用于约束后续实现：对标 Quick BI 小Q问数的产品思路，把问数结果从“万能大卡片”改为“用户消息 + AI 回答消息”交替出现的稳定对话流。
...
- 推荐追问和数据解读属于下一条派生问题入口，不塞进事实结果卡片内部。
- 用户能看懂每个区域的来源：事实来自 SQLBot 取数，解读来自 LLM 基于本轮结果生成。
- 旧数据可以自然降级：没有保存解读时只显示“数据解读”推荐动作，不自动补生成派生问题。
...
最终形态必须是问答交替的聊天消息流。每一次成功问数先形成一组“用户问题消息 + AI 事实回答消息”。事实回答消息下方可以展示推荐动作，但点击推荐动作后，必须继续生成下一条“用户/系统派生问题消息 + AI 派生回答消息”，不能在同一条 AI 回答下面堆一串 AI 消息卡片。
...
推荐动作不放在事实结果内部。默认顺序为用户问题、AI 事实回答、推荐动作、用户/系统派生问题、AI 派生回答。这样页面始终保持问答交替，不退化成一串连续 AI 消息卡片。
...
Quick BI 式的“做数据解读”更接近一个自动提问入口：用户点击后，系统自动插入一条派生问题消息，例如“对上面的结果做数据解读”，再以当前事实结果为上下文生成 AI 解读回答消息。它不是把问数回答文本改名成数据解读，也不是在历史恢复时自动重跑解读。
...
- 用户点击推荐动作后，在当前事实轮次下方追加派生问题消息和对应 AI 回答消息。
- 大屏宽度足够时，派生回答可以使用右侧辅助阅读面板同步展示；对话流中仍必须保留派生问题消息，避免历史恢复时丢失上下文。
...
- 如果后续产品决定默认开启解读，必须只在新问题提交链路触发一次，先创建 `kind = 'analysis'` 的派生问题记录，再写入 `analysis_status = generating`。

### 数据解读生成

1. 用户点击事实结果下方的“数据解读”推荐动作。
2. 前端在对话流中追加一条派生问题消息，问题文案可以由系统生成，例如“对上面的结果做数据解读”。
3. 派生问题携带 `sourceRecordId`，指向它要解读的事实问数记录。
4. 前端调用 SQLBot analysis 流接口。
5. 流式内容进入下一条 AI 解读回答消息，不覆盖事实结果区。
6. 生成完成后后端保存 `analysis`、`analysis_reasoning_content`、`analysis_record_id`、耗时、Token，并保存派生问题和源事实记录的关联关系。
7. 历史恢复时按消息流顺序直接展示派生问题消息、AI 解读回答消息和这些已保存字段。
...
2. 前端调用历史上下文/会话详情接口。
3. 后端返回完整会话快照。
4. 前端按消息流顺序恢复每一轮的用户问题、AI 事实回答、派生问题、AI 解读回答、AI 预测回答、推荐动作、执行上下文。
5. 前端禁止触发 `/analysis`、`/predict`、重新取图表数据等生成型请求，除非快照缺失必要事实数据且后端明确提供只读详情接口。

历史恢复规则：

- 有保存解读：立即恢复派生问题消息和它后面的 AI 解读回答消息。
- 没有保存解读：事实结果下方只展示“数据解读”推荐动作，不自动创建派生问题消息。
- 保存解读失败：恢复派生问题消息和失败状态，展示重试按钮。
- 历史中的推荐追问按当时快照展示，不能重新随机生成。
- 点击“重新解读”时才生成新解读，并覆盖或追加为该派生问题的最新解读版本。
...
- `GET /chat/{chat_id}/sqlbot-new/context` 或会话详情接口：返回完整会话快照，包含每轮用户消息、AI 回答消息、派生问题消息、派生消息与事实记录的关联、已保存解读。
...
- `SqlbotDerivedQuestionMessage.vue`：展示系统自动提出的派生问题消息，例如“对上面的结果做数据解读”。
- `SqlbotConversationTurn.vue`：组合用户问题消息、AI 事实回答消息、派生问题消息和 AI 派生回答消息，不包含生成逻辑。

事件约束：

- `SqlbotFollowupPanel` 只通过用户点击发出 `create-derived-question`。
- `SqlbotInsightPanel` 只在派生问题创建后发出或接收 `generate-analysis`。
...
- `none`：仅在派生问题消息已经创建但尚未返回内容时展示等待态；没有派生问题消息时不展示空解读卡片。
...
- 点击“数据解读”后，系统在事实结果下方追加一条派生问题消息，再只调用一次 `/analysis`，流式内容进入下一条 AI 解读回答消息，图表/表格不被覆盖。
- 历史中已经生成过数据解读时，必须恢复派生问题消息和 AI 解读回答消息，不能只恢复一段孤立解读文本。
- 刷新页面恢复当前会话时，不重复触发 `/analysis`。
- 推荐动作出现在 AI 事实回答消息下方，点击后必须作为明确用户动作生成派生问题消息。
...
- 新建问数，等待事实结果生成。
- 点击“数据解读”，确认系统追加派生问题消息，AI 解读回答在下一条消息中流式生成并持久化。
- 刷新页面，确认保存的解读直接恢复。
- 刷新页面后确认派生问题消息也被恢复，且 `sourceRecordId` 仍指向原事实记录。
...
3. 拆分事实结果、推荐动作、派生问题消息、解读回答消息、趋势预测消息组件。
4. 修改历史恢复，只读恢复快照。
