...
| Trend prediction alternates question then answer | PASS | `tmp/sqlbot-message-flow-qa/03-predict-derived-flow.png` | Original isolated `03-predict-derived-flow.har` was empty and treated as unreliable capture noise. A clean repro showed `POST /api/v1/chat/record/51/predict` returned 200, and the full accessibility snapshot showed user-visible DOM text `对“按品线统计销售金额”做趋势预测` followed by `趋势预测`. The `data-testid` locator was not used as passing evidence because it remained unreliable in the iframe/CORS context. |
...
- Original Case 3 HAR capture was unreliable because it saved with 0 entries; final Case 3 verification used the clean repro predict request and full accessibility snapshot user-visible text instead of the failing `data-testid` locator.
