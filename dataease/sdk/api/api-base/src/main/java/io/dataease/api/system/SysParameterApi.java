...
public interface SysParameterApi {
    ...
    @GetMapping("/sqlbotOld")
    @Operation(summary = "查询sqlbotOld嵌入配置")
    SQLBotConfigVO sqlBotOldConfig();

    @PostMapping("/sqlbotOld")
    @Operation(summary = "保存sqlbotOld嵌入配置")
    void saveSqlBotOldConfig(@RequestBody SQLBotConfigCreator configVO);
