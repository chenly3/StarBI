...
export interface AxiosInstanceWithLoading extends AxiosInstance {
  <T = any, R = AxiosResponse<T>, D = any>(
    config: AxiosRequestConfig<D> & AxiosConfigExtras
  ): Promise<R>
}
...
service.interceptors.response.use(
  (response: AxiosResponse<any> & { config: InternalAxiosRequestConfig & AxiosConfigExtras }) => {
    ...
    if (response.config.responseType === 'blob') {
      ...
      if (
        ...
        if (response.data.code === 80001) {
          ...
          router.push(`/login?redirect=${encodeURIComponent(queryRedirectPath)}`)
        }
