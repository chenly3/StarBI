...
function ensureComponentStyle(componentItem) {
  componentItem.style =
    componentItem.style && typeof componentItem.style === 'object' ? componentItem.style : {}
}

function ensureCommonBackground(componentItem) {
  if (!componentItem.commonBackground || typeof componentItem.commonBackground !== 'object') {
    componentItem.commonBackground = {}
  }
  if (componentItem.commonBackground.innerPadding === undefined) {
    componentItem.commonBackground.innerPadding = getNewInnerPadding()
  }
  if (componentItem.commonBackground.borderRadius === undefined) {
    componentItem.commonBackground.borderRadius = getNewBorderRadius()
  }
}

function ensureComponentEvents(componentItem) {
...
export function historyItemAdaptor(
  ...
  ensureComponentStyle(componentItem)
  ensureCommonBackground(componentItem)
  ...
  // 组件事件适配
  ensureComponentEvents(componentItem)
...
export function historyAdaptor(
  ...
  canvasDataResult.forEach(componentItem => {
    try {
      ...
      console.error('history adaptor failed for component', componentItem?.id, error)
    }
  })
}
...
export function initCanvasDataPrepare(dvId, params, callBack) {
  ...
  method(dvId, busiFlagCustom, attachInfo as { source: string; taskId: any }).then(res => {
    ...
    try {
      ...
      console.error('history adaptor failed', error)
    }
    const curPreviewGap =
