...
import {
  insertAIQueryChartIntoCanvas,
  listAIQueryChartResources,
  listAIQueryRecentResults
} from '@/api/aiQueryChartResource'
...
import SqlbotInsertDrawer from '@/views/dashboard/components/SqlbotInsertDrawer.vue'
...
const sqlbotInsertVisible = ref(false)
const sqlbotInsertRecentItems = ref<Array<Record<string, any>>>([])
const sqlbotInsertSavedItems = ref<Array<Record<string, any>>>([])
...
const locateInsertedComponent = (viewId: string | number, attempt = 0) => {
  const targetId = String(viewId || '')
  if (!targetId) {
    return
  }
  const index = dashboardComponentData.value.findIndex(item => String(item.id) === targetId)
  if (index < 0) {
    if (attempt < 12) {
      window.setTimeout(() => locateInsertedComponent(targetId, attempt + 1), 400)
    }
    return
  }
  const target = dashboardComponentData.value[index]
  dvMainStore.setCurComponent({ component: target, index })
  window.setTimeout(() => {
    deCanvasRef.value?.scrollTo?.(target?.y || 1)
  }, 400)
}

const openSqlbotInsert = async () => {
  sqlbotInsertVisible.value = true
  try {
    const [recentItems, savedItems] = await Promise.all([
      listAIQueryRecentResults(),
      listAIQueryChartResources()
    ])
    sqlbotInsertRecentItems.value = Array.isArray(recentItems) ? recentItems : []
    sqlbotInsertSavedItems.value = Array.isArray(savedItems) ? savedItems : []
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : '插入问数图表列表加载失败')
  }
}

const insertSqlbotChartIntoCurrentCanvas = async (
  sourceType: 'snapshot' | 'resource',
  ...
  try {
    const insertedViewId = await insertAIQueryChartIntoCanvas({
      sourceType,
      sourceId,
      targetCanvasType: 'dashboard',
      targetCanvasId: state.resourceId || dvInfo.value?.id
    })
    sqlbotInsertVisible.value = false
    ElMessage.success('问数图表已插入当前仪表板')
    wsCache.delete('DE-DV-CATCH-' + (state.resourceId || dvInfo.value?.id))
    initLocalCanvasData(() => {
      locateInsertedComponent(insertedViewId)
    })
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : '插入问数图表失败')
  }
}
...
onMounted(async () => {
  ...
  await Promise.race([
    new Promise(r => (p = r)),
    new Promise(resolve => window.setTimeout(resolve, XPACK_LOAD_TIMEOUT))
  ])
  ...
  const insertedViewId = String(router.currentRoute.value.query.insertedViewId || '')
  if (resourceId) {
    dataInitState.value = false
    const canvasCache = wsCache.get('DE-DV-CATCH-' + resourceId)
    if (insertedViewId) {
      wsCache.delete('DE-DV-CATCH-' + resourceId)
      initLocalCanvasData(() => {
        locateInsertedComponent(insertedViewId)
      })
    } else if (canvasCache) {
...
<template>
  ...
  >
    <DbToolbar @recoverToPublished="doRecoverToPublished" @openSqlbotInsert="openSqlbotInsert" />
    <el-container
  ...
  ></MobileConfigPanel>
  <SqlbotInsertDrawer
    :visible="sqlbotInsertVisible"
    :recent-items="sqlbotInsertRecentItems"
    :saved-items="sqlbotInsertSavedItems"
    @update:visible="value => (sqlbotInsertVisible = value)"
    @selectRecent="id => insertSqlbotChartIntoCurrentCanvas('snapshot', id)"
    @selectSaved="id => insertSqlbotChartIntoCurrentCanvas('resource', id)"
  />
  <XpackComponent
