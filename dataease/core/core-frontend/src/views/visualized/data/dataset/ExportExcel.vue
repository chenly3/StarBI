...
type ExportStatus = 'ALL' | 'IN_PROGRESS' | 'SUCCESS' | 'FAILED' | 'PENDING'
...
type ExportCenterParams = {
  activeName?: ExportStatus
}

type TabClickPane = {
  paneName?: ExportStatus
}
...
const handleClick = (tab?: TabClickPane) => {
  if (tab?.paneName) {
...
const init = (params?: ExportCenterParams) => {
  drawer.value = true
...
const taskExportTopicCall = task => {
  if (!linkStore.getLinkToken && !isDataEaseBi.value && !appStore.getIsIframe) {
    const exportTask = JSON.parse(task) as ExportTask
    if (exportTask.exportStatus === 'SUCCESS') {
      openMessageLoading(
        exportTask.exportFromName + ` ${t('data_set.successful_go_to')}`,
        'success',
        callbackExportSuc
      )
      return
    }
    if (exportTask.exportStatus === 'FAILED') {
      openMessageLoading(
        exportTask.exportFromName + ` ${t('data_set.failed_go_to')}`,
        'error',
        callbackExportError
      )
    }
...
<template>
  ...
  >
    <div v-loading="drawerLoading" class="export-excel-body">
