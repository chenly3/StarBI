...
interface Form {
  ...
  type: string
  copy?: boolean
  configuration?: Partial<Configuration> & AnyRecord
  apiConfiguration?: (ApiConfiguration & AnyRecord)[]
  paramsConfiguration?: (ApiConfiguration & AnyRecord)[]
...
const handleSavedDatasource = (res: AnyRecord) => {
  handleShowFinishPage({ id: res.id, name: res.name, pid: res.pid })
}
...
const doValidateDs = request => {
  ...
  if (currentDsType.value === 'ExcelRemote') {
    let excelRequest = JSON.parse(JSON.stringify(form2.configuration)) as AnyRecord
...
const saveDS = () => {
  ...
  > & {
    configuration: string
    apiConfiguration: (ApiConfiguration & AnyRecord)[] | string
    paramsConfiguration?: (ApiConfiguration & AnyRecord)[]
    syncSetting?: SyncSetting
  }
  ...
  if (currentDsType.value === 'ExcelRemote') {
    ...
    const apiConfiguration = request.apiConfiguration as (ApiConfiguration & AnyRecord)[]
    for (let i = 0; i < apiConfiguration.length; i++) {
      if (
        apiConfiguration[i].deTableName === '' ||
        apiConfiguration[i].deTableName === undefined ||
        apiConfiguration[i].deTableName === null
      ) {
        apiConfiguration[i].deTableName =
          'api_' + apiConfiguration[i].name + '_' + uuid.v1().replaceAll('-', '').substring(0, 10)
      }
      apiConfiguration[i].jsonFields = []
      for (let j = 0; j < apiConfiguration[i].fields.length; j++) {
        apiConfiguration[i].fields[j].value = []
      }
    }
    let apiItems = []
    apiItems = apiItems.concat(apiConfiguration)
    if (request.paramsConfiguration) {
      apiItems = apiItems.concat(request.paramsConfiguration)
    }
    request.configuration = Base64.encode(JSON.stringify(apiItems))
...
const defaultForm2: Param & AnyRecord = {
  type: '',
  id: '0',
  editType: 0,
  name: '',
