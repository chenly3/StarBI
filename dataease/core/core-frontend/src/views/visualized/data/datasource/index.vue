...
type DatasourceNode = Omit<Node, 'copy' | 'configuration' | 'syncSetting' | 'apiConfiguration'> & {
  configuration: Node['configuration'] | null | Record<string, any>
  syncSetting: Node['syncSetting'] | null
  apiConfiguration: Array<Record<string, any>>
  paramsConfiguration?: Array<Record<string, any>>
  copy?: boolean
...
const validateDS = () => {
  let nodeTmpInfo = reactive<DatasourceNode>(cloneDeep(defaultInfo))
  Object.assign(nodeTmpInfo, cloneDeep(nodeInfo))
  validateById(nodeTmpInfo.id as number)
    .then(res => {
      if (res.data.type.startsWith('API')) {
        let error = 0
        const dsStatus = JSON.parse(res.data.status) as ApiStatus[]
...
const nodeInfo = reactive<DatasourceNode>(cloneDeep(defaultInfo))
...
const editDatasource = (editType?: number) => {
  ...
  return getById(nodeInfo.id).then(res => {
    ...
    let datasource = reactive<DatasourceNode>(cloneDeep(defaultInfo))
