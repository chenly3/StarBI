...
const normalizeTitleStyle = (customStyle: any) => {
  const mergedTitleStyle = {
    ...DEFAULT_TITLE_STYLE,
    ...(customStyle?.text || {})
  }
  if (mergedTitleStyle.letterSpace === null || mergedTitleStyle.letterSpace === undefined) {
    mergedTitleStyle.letterSpace = DEFAULT_TITLE_STYLE.letterSpace
  }
  customStyle.text = mergedTitleStyle
  return mergedTitleStyle
}

const initTitle = () => {
  if (view.value.customStyle) {
    const customStyle = view.value.customStyle
    const textStyle = normalizeTitleStyle(customStyle)
    if (textStyle) {
      state.title_show = textStyle.show
      state.title_class.fontSize = textStyle.fontSize * scale.value + 'px'
      state.title_class.color = textStyle.color
      state.title_class.textAlign = textStyle.hPosition as CSSProperties['textAlign']
      state.title_class.fontStyle = textStyle.isItalic ? 'italic' : 'normal'
      state.title_class.fontWeight = textStyle.isBolder ? 'bold' : 'normal'
      if (!!appearanceStore.fontList.length) {
        appearanceStore.fontList.forEach(ele => {
          CHART_FONT_FAMILY_MAP[ele.name] = ele.name
        })
      }
      state.title_class.fontFamily = textStyle.fontFamily
        ? CHART_FONT_FAMILY_MAP[textStyle.fontFamily]
        : DEFAULT_TITLE_STYLE.fontFamily
      if (!CHART_FONT_FAMILY_MAP[textStyle.fontFamily]) {
        state.title_class.fontFamily = appearanceStore.fontList.find(ele => ele.isDefault)?.name
        textStyle.fontFamily = state.title_class.fontFamily
      }
      appearanceStore.setCurrentFont(state.title_class.fontFamily)
      state.title_class.letterSpacing =
        (textStyle.letterSpace ? textStyle.letterSpace : DEFAULT_TITLE_STYLE.letterSpace) + 'px'
      state.title_class.textShadow = textStyle.fontShadow ? '2px 2px 4px' : 'none'
    }
    ...
    state.title_remark.show = textStyle.show && textStyle.remarkShow
    state.title_remark.remark = textStyle.remark
  }
}
