...
import SqlbotInsertDrawer from '@/views/data-visualization/components/SqlbotInsertDrawer.vue'
import {
...
const insertSqlbotChartIntoCurrentCanvas = async (
  ...
  try {
    await insertAIQueryChartIntoCanvas({
      sourceType,
      sourceId,
      targetCanvasType: 'dataV',
      targetCanvasId: state.dvInfo.id
    })
    sqlbotInsertVisible.value = false
    ElMessage.success('问数图表已插入当前大屏')
    loadCanvasData(state.dvInfo.id, state.dvInfo.weight, state.dvInfo.ext)
  } catch (error) {
    ...
    >
      ...
      <template v-if="dvInfo.name">
        <preview-head
          ...
          @openSqlbotInsert="openSqlbotInsert"
        />
        <div
