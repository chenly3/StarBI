...
import { buildDashboardQueryRoute } from '@/views/sqlbot/queryContext'
...
const emit = defineEmits(['reload', 'download', 'downloadAsAppTemplate', 'openSqlbotInsert'])
const { t } = useI18n()
...
const previewInfo = computed<Record<string, any>>(() => (dvInfo.value || {}) as Record<string, any>)
const reload = () => {
...
const openDashboardQueryPage = () => {
  router.push(
    buildDashboardQueryRoute({
      dashboardId: dvInfo.value.id,
      dashboardName: dvInfo.value.name,
      componentData: dvMainStore.componentData,
      sourceWidget: 'dashboard-preview-head'
    })
  )
}

const openSqlbotInsert = () => {
  emit('openSqlbotInsert')
}
storeQuery()
watch(
  () => dvInfo.value.id,
  ...
  <div class="preview-head flex-align-center">
    ...
    <div class="create-area flex-align-center">
      <span style="line-height: 22px"
        >{{ t('visualization.creator') }}:{{ previewInfo.creatorName }}</span
      >
    ...
    <div class="canvas-opt-button">
      <el-button secondary :disabled="dvInfo.status === 0" @click="openSqlbotInsert">
        插入问数图表
      </el-button>
      <el-button secondary :disabled="dvInfo.status === 0" @click="openDashboardQueryPage">
        {{ t('starbi.dashboard_query_entry') }}
      </el-button>
      <el-button
        v-if="!isIframe"
      ...
      <ShareVisualHead
        ...
        :weight="previewInfo.weight"
        :resource-type="dvInfo.type"
      />
      <el-button
        class="custom-button"
        v-if="previewInfo.weight > 6"
        type="primary"
        @click="dvEdit()"
      >
        <template #icon>
