...
const edit = ({ id, domain, secret, valid, enabled }) => {
  state.form = {
    id,
    domain,
    secret,
...
const save = () => {
  ...
  const method = request.post({ url: props.apiPath, data: param })
  showLoading()
  method
    .then(res => {
      if (!res.msg) {
...
<template>
  <el-drawer
    :title="props.drawerTitle || t('common.sqlbot_settings')"
  ...
  >
    ...
    >
      ...
      <el-form-item :label="$t('common.sqlbot_secret')">
        <el-input
          v-model="state.form.secret"
          show-password
          :placeholder="t('starbi.sqlbot_secret_placeholder')"
        />
      </el-form-item>
    </el-form>
    <template #footer>
