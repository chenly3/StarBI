...
const replaceHashRedirect = (target: unknown) => {
  if (typeof window === 'undefined') {
    return router.push(toRedirectLocation(target))
  }
  const normalized = normalizeRedirectPath(target)
  const nextUrl = new URL(window.location.href)
  nextUrl.hash = normalized.startsWith('/') ? `#${normalized}` : `#/${normalized}`
  window.location.replace(nextUrl.toString())
}

const persistLoginRedirectGuard = (targetPath: string) => {
  const rawRedirect = normalizeRedirectPath(router.currentRoute.value.query.redirect, '')
  if (!rawRedirect || rawRedirect === '/workbranch/index') {
    sessionCache.delete(LOGIN_REDIRECT_GUARD_KEY)
    return
  }

  const [guardPath] = rawRedirect.split('?')
  sessionCache.set(LOGIN_REDIRECT_GUARD_KEY, {
    path: guardPath || targetPath,
    createdAt: Date.now()
  })
}

const getCurLocation = () => {
  return normalizeRedirectPath(router.currentRoute.value.query.redirect)
}
      ...
      loginApi(param)
        .then(res => {
          ...
          if (!isLdap && !xpackLoadFail.value && xpackInvalidPwd.value?.invokeMethod) {
            ...
            const handled = xpackInvalidPwd?.value.invokeMethod(param)
            if (handled) {
              return
            }
          }
          ...
          persistLoginRedirectGuard(queryRedirectPath)
          replaceHashRedirect(queryRedirectPath)
        })
        .catch(() => {
          duringLogin.value = false
        })
        ...
        <div class="login-form-center">
          ...
          >
            <div class="login-logo">
              <StarBILogo v-if="!loginLogoUrl && axiosFinished" class="login-logo-brand" />
